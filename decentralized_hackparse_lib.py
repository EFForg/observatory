...
def preprocess_lines(cstring):
  """This function exists to convert openssl x509 -text output format to old version
     of openssl, when this script was written. Yuck.
  """
  newlines = []
  lines = cstring.split("\n")
  for line in lines:
    ...
    if l.startswith("Public-Key:"):
      line = line.replace("Public-Key", "RSA Public Key")
      line = line[4:]
    newlines.append(makelinestruct(line))
  return newlines
...
def hacky_parse(cstring):
  ...
  # preprocess to use new openssl version
  lines = preprocess_lines(cstring)
  prev_indent = -1 
  results = {}
  value = ""
  ...
  for n,(i,line) in enumerate(lines):
    ...
    # tododta remove
    #print "ITERATION %s: i=%s; bottom=%s; STRUCT_INDENTS=%s; prev=%s" % (n, i, bottom, struct_indents, prev_indent)
    #print line
    ...
    if i < prev_indent:
      ...
      print "TMP struct_indents: %s" % struct_indents
...
def add_field(fieldname, fieldtype='TEXT'):
  "Adds a field, cleaning it up, altering the DB and appending to allfields"
  allfields.append(fieldname)
  q = "ALTER TABLE %s ADD COLUMN `%s` %s" % (gdb.escape_string(tablename), \
      gdb.escape_string(fieldname.strip()), fieldtype)
  print q
  try:
    gdbc.execute(q)
...
def add_cert_to_db(cert_fp, validities, pprinted_certchain, fprints):
  ...
  for i, pprinted_cert in enumerate(pprinted_certchain):
    print "parsing", cert_fp
    fields = hacky_parse(pprinted_cert)
    if create:
      add_field('cert_fp', 'binary(36)');
    fields['cert_fp'] = cert_fp
