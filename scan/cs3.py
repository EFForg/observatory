...
def markNextTarget(scan_gaps=False):
  ...
  # we're doing it.
  dbc.execute("LOCK TABLES spaces WRITE, spaces AS p2 WRITE, spaces AS p3 WRITE")
  try:
    if not scan_gaps:
...
SELECT s8, 
       s32 +1 AS hole, 
       (
         SELECT MIN(p2.s32) FROM spaces AS p2 
         WHERE p2.s8=spaces.s8 AND p2.s32 > spaces.s32
       ) as next 
FROM spaces 
